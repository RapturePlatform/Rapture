...
function error(msg) {
  ...
  log(backtrace());
  UnsupportedManager.notify(UNSUPPORTED_PLUGINS.unknown);
  throw new Error(msg);
}
...
var UNSUPPORTED_PLUGINS = PDFJS.UNSUPPORTED_PLUGINS = {
  unknown: 'unknown',
  forms: 'forms',
...
var UnsupportedManager = PDFJS.UnsupportedManager =
  ...
  return {
    ...
    notify: function (pluginId) {
      warn('Unsupported plugin "' + pluginId + '"');
      for (var i = 0, ii = listeners.length; i < ii; i++) {
        listeners[i](pluginId);
      }
    }
  };
})();
...
function MessageHandler(name, comObj) {
  ...
  ah['_unsupported_plugin'] = [function ah_unsupportedPlugin(data) {
...
var Pattern = (function PatternClosure() {
  ...
  Pattern.parseShading = function Pattern_parseShading(shading, matrix, xref,
    ...
    switch (type) {
      ...
      default:
        UnsupportedManager.notify(UNSUPPORTED_PLUGINS.shadingPattern);
