...
options {
    ...
    output=template;
    superClass = AbstractTTreeDotNet;
}
...
crudTypeExpr scope { 
    ...
    String apiFullPath = "Generated/crud/" + $name.text + ".cs";
...
typeExpr
    ...
    ^(TYPEDEF doc=DOC 
        ...
        |   indexedAnnotation {
                $typeExpr::indexed = $indexedAnnotation.result;
            }
        |   beanAnnotation {
        ...
        name=ID typeAspect typeFields) {
            ...
            addType($name.text, $typeAspect.name, $typeExpr::bean, $typeExpr::cacheable, $typeExpr::addressable, $typeExpr::storable, $typeExpr::extend, $typeExpr::deprecated, $typeExpr::indexed, sdkName, $typeFields.fieldNameToType, 
...
typeFieldDef returns [String fieldName, StringTemplate beanInfo, String varType, String constructor]
    ...
    ^(TYPEMEMBER 
        ...
        $fieldName = $name.text;
        String transportName = $fieldName;
        String realFieldName = convertFieldName($fieldName);
        $beanInfo = %beanInfo(varName={realFieldName},
        transportName={$fieldName},
...
vartype returns [String mainType, boolean isMappedType, String importType, String defValue, boolean equalsTest, String genericAlt]:
    ...
    | ^(RAW DATETYPE) { $mainType = "DateTime"; $isMappedType = false; $importType = ""; $defValue = "null"; $equalsTest=true;}
    ...
    | ^(RAW BOOLTYPE) { $mainType = "bool"; $isMappedType = false; $importType = ""; $defValue = "true";$equalsTest=true;}
    ...
    | ^(SETTYPE v=vartype) { 
            $isMappedType = $v.isMappedType; $importType = $v.mainType; $defValue="null"; $equalsTest=true;
            $mainType = String.format("HashSet<\%s>", $v.genericAlt == null ? $v.mainType : $v.genericAlt);
        }
    | ^(MAPTYPE k=vartype v=vartype) { 
            $isMappedType = $v.isMappedType; $importType = $v.mainType; $defValue="null";$equalsTest=true;
            $mainType
                = String.format("Dictionary<\%s, \%s>", $k.genericAlt == null ? $k.mainType : $k.genericAlt,
    ...
    | ^(COMPLEX typeid=ID) {
            ...
            if ($mainType.endsWith("URI")) {
                $genericAlt = "String";
            } else if ($mainType.equals("Integer")) {
            	$mainType = "int"; $isMappedType = false; $importType = ""; $defValue = "0"; $equalsTest=true;
            }
            else {
                $genericAlt = $mainType;
            }
...
:
      ^(APISEC doc=DOC deprecated=deprecatedAspect? apiType=ID { $apiExpr::apiName = convertApiName($apiType.text); } (a=apilist)) {
      ...
      // need to emit at this level the DispatchFunction and the API definition
            ...
            String convertedName = convertApiName($apiType.text);
      ...
      StringTemplate s = %apilist(sdkname={sdkName}, apitype={convertedName}, apis={$a.main},typeimports={imp});
      ...
      StringTemplate scriptS = %scriptapilist(sdkname={sdkName}, apitype={convertedName}, apis={$a.scriptMain}, typeimports={imp});
      ...
      StringTemplate s3 = %httpfile(sdkname={sdkName}, apitype={convertedName}, imports={imp}, fns={$a.httpApi},  scriptFns={$a.httpScriptApi}, payloadImports={$a.payloadImports});
...
: ^(APIENTRY doc=DOC ^(ENTITLEASPECT (rent=REGULARENTITLE {regularEntitlements.add($rent.text);})+ (k=ID v=ID {dynamicEntMap.put($k.text, $v.text);})*) deprecated=deprecatedAspect? streaming=STREAMING? ^(VISIBILITY visibility) ^(RETTYPE vartype) ^(FNNAME name=ID) p=paramList) {
    ...
    addCallNameEntry(%callNameEntry(api={$apiExpr::apiName}, name={$name.text}));
        ...
        $wrapperStatement = %wrapperEntry(ret={$vartype.mainType},name={$name.text},params={$p.main}, callParams={$p.paramListAspect}, apitype={$apiExpr::apiName}, deprecated={$deprecated.reason});
        
        $scriptEntry = %apiEntry(ret={$vartype.mainType},name={$name},params={$p.scriptMain}, deprecated={$deprecated.reason});
        $scriptImpl = %scriptapientry(ret={$vartype.mainType}, name={$name}, params={$p.scriptMain}, apiparams={$p.paramListAlt});
        // And this is the one that is used to create new files
        String retImport = "";
        if ($vartype.isMappedType) {
          retImport = "using " +  getPackageAndClass($vartype.importType) + ";";
        ...
        if (!isPrivate && !isStreaming) {
            ...
            $httpScriptEntry = %httpscriptapifn(apitype={$apiExpr::apiName}, ret={$vartype.mainType}, name={$name.text}, params={$p.scriptMain}, callParams={$p.paramListAlt});
                              
          $httpEntry = %httpapifn(apitype={$apiExpr::apiName}, ret={$vartype.mainType}, name={$name.text}, params={$p.main}, apiparams={$p.paramList});
                    
                    
          // Also create a payload file
          StringTemplate s2 = %payloadFile(sdkname={sdkName}, apitype={$apiExpr::apiName}, name={$name.text}, params={$p.alternate}, apiparams={$p.paramList}, imports={$p.packageList}, retImport={retImport}, dynEnt={dynEntEntries});
          String reqFullPath2 = getFileNameForType(sdkName, "payload/" + $apiExpr::apiName, $name.text, "Payload");
...
param returns[StringTemplate alternate, StringTemplate param, StringTemplate pack, StringTemplate testSetter, StringTemplate testGetter, String typeName]:  ^(PARAM ^(TYPE t=vartype) ^(NAME n=ID) BANG?) {
     String transportName = $n.text;
     String realName = convertFieldName($n.text);
     $st = %param(type={$t.mainType},name={realName});
     ...
     $alternate = %attrForClass(name={realName},
        transportName={transportName},
		type={(isURI) ? "string" : $t.mainType},
		isURI={isURI},
		scheme={scheme});
     $param = %param2(name={realName});
     ...
     $testSetter = %callSetter(name={realName}, value={$t.defValue});
     $testGetter = %testGetter(name={realName}, value={$t.defValue}, equalsTest={$t.equalsTest});
     };
