...
hmxdef returns[List vals, List headers]
@init {
	$vals = new ArrayList<StringTemplate>();
	$headers = new ArrayList<StringTemplate>();
} 	: ^(MAIN versionExpr minVerExpr
      (
          ^(INNERT typeExpr) 
        | ^(INNERA (a=apiExpr { $headers.add($a.header); $vals.add($a.apisec); } ))
      )*
      )
      {
      
						 StringTemplate s 
						  = %JSapi(apiHeader={$headers}, apilist={$vals}, versionMajor={$versionExpr.major}, versionMinor={$versionExpr.minor}, versionMicro={$versionExpr.micro});
           				 String apiFullPath = "rapture.js";
...
apiExpr returns[List apisec, StringTemplate header] 
...
:
			^(APISEC DOC deprecated=deprecatedAspect? apiType=ID { 
						 $apiExpr::apiName = $apiType.text; 
						 $header = %JSExportSetup(apiType={$apiType.text}); 
						 } (a=apilist)) {
...
: ^(APIENTRY DOC  ^(ENTITLEASPECT (rent=REGULARENTITLE)+ (k=ID v=ID)*) deprecatedAspect? ^(VISIBILITY visibility) ^(RETTYPE vartype) ^(FNNAME name=ID) p=paramList) {
			$st = %JSApiEntry(apitype={$apiExpr::apiName},name={$name.text},paramNames={$p.names}, params={$p.main});
		};
...
param returns[String name] : ^(PARAM ^(TYPE t=vartype) ^(NAME n=ID) b=BANG?) {
   boolean val = $b.text == null ? false : true;
	 $st = %JSParam(name={$n.text}, bang={val});
	 $name = $n.text;
	 };
